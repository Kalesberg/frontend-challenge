  ...
  async updateTokenList() {
    ...
    if (networkID === 1) {
      try {
        ...
        console.error(
          `Token list fetch failed, search by address can still be used ${e}`
        );
      }
    }
  }
  ...
  async refreshTokenInfo(token, accountAddress) {
    ...
    if (token.address === Addresses.eth.toLowerCase()) {
      ...
      token.balance = new BigNumber(
        await web3Modal.eth.getBalance(accountAddress)
      ).dividedBy(10 ** token.decimals);
    } else {
      let contract = new web3Modal.eth.Contract(Erc20Abi, token.address);

      token.symbol = await contract.methods
        .symbol()
        .call()
        .then((s) => s.toUpperCase())
        .catch((e) => console.log(e));

      token.decimals = await contract.methods
        .decimals()
        .call()
        .then((s) => parseInt(s));

      token.balance = new BigNumber(
        await contract.methods.balanceOf(accountAddress).call()
      ).dividedBy(10 ** token.decimals);
    }
  }
}
